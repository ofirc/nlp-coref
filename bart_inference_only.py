...
def main():
    ...
    model_params = {
        "MODEL": "model_files_checkpoint_epoch3",
    ...
    train_df = train_df.apply(lambda s: s.str.replace('^coref: ', ''))
    dev_df = dev_df.apply(lambda s: s.str.replace('^coref: ', ''))
    test_df = test_df.apply(lambda s: s.str.replace('^coref: ', ''))
