...
class CorefDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        document_id = self.data.iloc[index]["id"]
...
def generate_text_on_test_set(epoch, tokenizer, model, device, loader):
  ...
  return predictions, actuals, document_ids
...
def T5Trainer(
    ...
    for epoch in range(model_params["TEST_EPOCHS"]):
        predictions, actuals, document_ids = generate_text_on_test_set(epoch, tokenizer, model, device, test_loader)
...
def main():
    ...
    model_params = {
        "MODEL": "model_files_checkpoint_epoch3",  # model_type: t5-base/t5-large
        "TRAIN_BATCH_SIZE": 2,  # training batch size
        "DEV_BATCH_SIZE": 2,  # dev batch size
        "TEST_BATCH_SIZE": 4,  # validation batch size
    ...
    assert train_df.iloc[0]["input"].startswith('coref: ')
    assert dev_df.iloc[0]["input"].startswith('coref: ')
    assert test_df.iloc[0]["input"].startswith('coref: ')
